...
public class TypeOfResponsibilityController {
    ...
    private int delete() throws SQLException, IOException {
        ...
        exeptionScene.createDeleteScene(typeOfResponsibility.getName());
    ...
    private int update() throws SQLException, IOException {
        ...
        if(typeField.getText().isEmpty() && groupCombobox.getSelectionModel().getSelectedIndex() == 0){
            ...
            exeptionScene.createEditHardScene(typeOfResponsibility.getId().toString());
            while (DeleteAndEditController.isWaiting) {
            }
            //Запрос в БД
            if(DeleteAndEditController.needDelete){
    ...
    private void loadCombobox(Connection connection) throws SQLException {
        ObservableList<String> list = FXCollections.observableArrayList();
        list.add("Выберите группу ответственности");
        String getGroups = "SELECT name FROM responsibility_group";
